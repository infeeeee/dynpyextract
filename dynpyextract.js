...
/**
 * Returns python nodes from input files
...
/**
 * 
 * @param {Array} pythonFiles Python from files
 * @param {String} output The path to output
 */
    ...
    return new Promise((resolve, reject) => {
        ...
        if (outputType == 'file' && pythonFiles.length > 1) {
            ...
            if (pythonFiles[0].length == 1) {
                // only one python node in dyn file:
                fs.writeFileSync(output, pyOutput(pythonFiles[0][0]))
            } else {
                // multiple pythonnodes in file
                for (let i = 0; i < pythonFiles[0].length; i++) {
                    ...
                    fs.writeFileSync(writePath, pyOutput(pythonFiles[0][i]))
                }
            }
        }
...
/**
 * The type of the output: folder or files
 * @param {String} output The output path
 * @returns {String} the type of output: 'folder', 'file' or false
 */
...
/**
 * Generate header and nice output for python files
 * @param {Object} pythonData The object of python data
 * @returns {String} python block with header as string
 */
function pyOutput(pythonData) {
    
    let outStrings = []

    outStrings.push('')
    outStrings.push('This file was generated by dynpyextract.')
    outStrings.push('More info: https://github.com/infeeeee/dynpyextract')
    outStrings.push('Do not modify this header if you want to update Dynamo files automatically.')
    outStrings.push('')
    outStrings.push('DpeVersion: ' + pythonData.dpeVersion)
    outStrings.push('DynFilePath: "' + pythonData.path + '"')
    outStrings.push('DynFileUuid: ' + pythonData.uuid)
    outStrings.push('NodeId: ' + pythonData.id)
    outStrings.push('')

    const stringLength = Math.max(...(outStrings.map(el => el.length)));

    outStrings.forEach((element, i) => {
        outStrings[i] = element.padEnd(stringLength)
        outStrings[i] = '### ' + outStrings[i] + ' ###\r\n'
    });

    let hashtag = '#'.repeat(stringLength + 8) + '\r\n'

    outStrings.unshift(hashtag)
    outStrings.push(hashtag)
    outStrings.push('\r\n')

    output = outStrings.join('')
    output += pythonData.code
...
program.command('update <input> [output]')
    ...
    .action(function (inputPath, ouputPath) {
        console.log('This functionality is not working yet...')
    })
